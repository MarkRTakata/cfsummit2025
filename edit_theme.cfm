...
<head>
    ...
    <title>ColdFusion Chart Theme Editor</title>
    <!-- Adobe Spectrum CSS -->
    ...
    <style>
        ...
        .theme-selection { background: #fff; padding: 20px; border: 2px solid #0078d4; border-radius: 4px; margin-bottom: 30px; }
        .save-buttons { display: flex; gap: 15px; justify-content: center; flex-wrap: wrap; }
    </style>
</head>
<body class="spectrum">
...
<cfparam name="form.saveAsNew" default="false">

<cfif len(form.selectedTheme) AND form.action NEQ "save">
    <cffile action="read" file="#themesDir#/#form.selectedTheme#" variable="themeContent">
    <cfset selectedTheme = deserializeJSON(themeContent)>
    <cfset showForm = true>
    <cfset currentThemeName = listFirst(form.selectedTheme, ".")>
<cfelseif form.action EQ "save" AND len(form.themeName)>
    <!-- Save the theme -->
    <!-- Start with the original theme structure -->
    <cffile action="read" file="#themesDir#/#form.selectedTheme#" variable="originalThemeContent">
    <cfset updatedTheme = deserializeJSON(originalThemeContent)>

    <!-- Update theme name -->
    <cfset updatedTheme["theme-name"] = form.themeName>

    <!-- Process all form fields back into theme structure -->
    <cfloop collection="#form#" item="fieldName">
        <cfif fieldName NEQ "selectedTheme" AND fieldName NEQ "action" AND fieldName NEQ "themeName" AND fieldName NEQ "saveAsNew" AND fieldName NEQ "fieldnames" AND fieldName NEQ "palette_data">
            <cfset fieldPath = listToArray(fieldName, "_")>
            <cfset currentLevel = updatedTheme>
    ...
    <cfif structKeyExists(form, "palette_data") AND len(trim(form.palette_data))>
        <cftry>
            <cfset updatedTheme["palette"] = deserializeJSON(form.palette_data)>
            <cfcatch>
    ...
    <cfset updatedTheme = cleanBooleanValues(updatedTheme)>

    <!-- Determine save filename -->
    <cfif form.saveAsNew EQ "true">
        <cfset saveFilename = form.themeName & ".json">
    <cfelse>
        <cfset saveFilename = form.selectedTheme>
    </cfif>

    <!-- Use ColdFusion's built-in JSON serialization -->
    <cfset themeJSON = serializeJSON(updatedTheme)>
    <cffile action="write" file="#themesDir#/#saveFilename#" output="#themeJSON#">

    <cfif form.saveAsNew EQ "true">
        <cfset savedMessage = "Theme '#form.themeName#' saved as new theme successfully!">
    <cfelse>
        <cfset savedMessage = "Theme '#form.themeName#' updated successfully!">
    </cfif>
</cfif>
    ...
    <div class="spectrum-Page-content theme-builder-container">
        ...
        <cfif NOT showForm>
            ...
            <div class="theme-selection">
                <h2 class="spectrum-Heading spectrum-Heading--sizeL">Select a Custom Theme to Edit</h2>
                <p class="spectrum-Body">Choose a custom theme from your themes folder to edit.</p>

                <cfif arrayLen(customThemeList) EQ 0>
                    <div class="spectrum-Alert spectrum-Alert--info">
                        <p>No custom themes found in the themes directory. <a href="builder.cfm">Create a theme first</a> using the Theme Builder.</p>
                    </div>
                <cfelse>
                    <form method="post">
                        <div class="field-item">
                            <label for="selectedTheme" class="spectrum-FieldLabel">Custom Theme:</label>
                            <select name="selectedTheme" id="selectedTheme" class="spectrum-Dropdown" required>
                                <option value="">-- Select a Theme --</option>
                                <cfoutput>
                                    <cfloop array="#customThemeList#" index="themeName">
                                        <cfset displayName = listFirst(themeName, ".")>
                                        <option value="#themeName#">#displayName#</option>
                                    </cfloop>
                                </cfoutput>
                            </select>
                        </div>
        ...
        <cfelse>
            <!-- Theme editing form -->
            <div class="theme-selection">
                <h2 class="spectrum-Heading spectrum-Heading--sizeL">Editing Theme: <cfoutput>#currentThemeName#</cfoutput></h2>
                <a href="edit_theme.cfm" class="spectrum-Button spectrum-Button--secondary">← Back to Theme Selection</a>
            </div>

            <form method="post" id="themeForm">
                <input type="hidden" name="action" value="save">
                <input type="hidden" name="selectedTheme" value="<cfoutput>#form.selectedTheme#</cfoutput>">
                <input type="hidden" name="palette_data" id="paletteData" value="">
                <input type="hidden" name="saveAsNew" id="saveAsNew" value="false">
                ...
                <div class="save-section">
                    <h3 class="spectrum-Heading spectrum-Heading--sizeM">Save Your Theme</h3>
                    <div class="theme-name-input">
                        <label for="themeName" class="spectrum-FieldLabel">Theme Name:</label>
                        <input type="text" name="themeName" id="themeName" class="spectrum-Textfield"
                               value="<cfoutput>#currentThemeName#</cfoutput>" required>
                    </div>
                    <div class="save-buttons">
                        <button type="submit" class="spectrum-Button spectrum-Button--cta spectrum-Button--sizeL" onclick="setSaveAsNew(false)">
                            Update Theme
                        </button>
                        <button type="submit" class="spectrum-Button spectrum-Button--secondary spectrum-Button--sizeL" onclick="setSaveAsNew(true)">
                            Save As New Theme
                        </button>
                    </div>
                </div>
...
function setSaveAsNew(isNew) {
    document.getElementById('saveAsNew').value = isNew ? 'true' : 'false';
}
